...
- **Overview** — a read-only, workspace-wide dashboard. Default landing view.
- **Graph** — the Cytoscape force-directed visualisation.
- **Workspace (Work)** — the node list on the left and, in the centre, the selected node's detail or its task chat. Where most daily work happens.
...
Every row is a link: clicking a node reference switches to Graph and selects it; clicking a session reference opens its node in Workspace and focuses that session. Nothing here is fetched automatically on an interval — use "Refresh".
...
- **Sidebar header** (every tab) — the workspace switcher under the brand, the Overview / Graph / Work toggle, "Search nodes…" (also `⌘K` / `Ctrl+K`) opening a command palette that filters nodes by name, description and type (a longer result list is grouped by node type — Organization, Area, Project, Process, Principle — a short one shows the type beside each name; ↑↓ / Enter / Esc hints sit in the footer), and "New node". The block is identical on every tab; in Graph a pick selects the node in the graph, in Work it opens the node.
- **Node list** (left, "Open") — two arrangements, switched by the Nodes | Status toggle (remembered per workspace). **Nodes**: every open node in the order you opened them, with a node-type dot and at most one activity dot (waiting on an answer or running, from live `session_state` frames — a suspended or new thread shows none); its threads (including an empty, just-opened draft — your own drafts follow you into another window of the same account as soon as that window refreshes the node's threads; nobody else ever sees them) sit as sub-rows under the node name, the state in the tooltip — click one to jump straight to the chat, double-click to rename inline, and the `×` revealed on hover closes it (a draft is deleted outright; anything else is Close thread, without a dialog). Sessions opened by hand from a CLI are not threads; they stay on the node's Threads tab. Exactly one row is marked: the thread the centre column is showing, or, when it shows the node itself, the node. A thread started anywhere else — the detail pane's "New task", the Threads tab's "Continue from handoff", another window — appears here as soon as its first `session_state` frame arrives, without reopening the node. Hovering a node itself shows `+` (opens a new, empty thread on that node — the same one-click action as the detail pane's "New task") and `×` (close the node — its sessions keep running on the sidecar). **Status**: tasks only, grouped Need attention / Working / Suspended / New / Done, each with its node's name underneath; a new, running or suspended task has the same hover `×` as in Nodes.
- **Centre** — [the task chat](#task-chat) when the selected node has an open (running/suspended/draft) session; otherwise the same `DetailPane` the graph view uses, in "embedded" mode.
...
- **Sharing** — the visibility selector (Everyone / Private / Group). A node that inherits a group ACL from its organization (or nearest restricted ancestor) shows a single read-only summary ("Inherits sharing from ...", the effective mode, the inherited recipients) with one action, "Set own sharing for this node"; that opens a card prefilled from the inherited list, and nothing is sent to the server until "Save" ("Cancel" discards it). Once a node has its own list — its own override, or a group set directly on a node with no restricted ancestor — the same card autosaves every change immediately, with a brief "Saved" readout, plus a "Remove own sharing and inherit from the organization" action to drop the override. The two destructive cases — switching away from a group with existing grants, or removing its last recipient — ask for an inline confirmation first. A group with no recipients yet is the only unsaved autosave state; it persists once the first recipient is added.
...
- **Files** — list of tracked files with `remote_path` and the derived `local_path` for this device, plus a sync button (team workspaces only — a personal workspace has no remote at all, see [Files: the two sync planes](/concepts/data-modes/)) that mounts as soon as the node has a mirror, even with nothing tracked yet. It's never disabled: with something to push/pull it reads "Sync (3 files to sync)"; once nothing is locally pending it reads "Check the remote" instead of claiming the node is done — a run's remote sweep is the only way to notice a file that showed up on Drive out of band, so a freshly created, still-empty mirror needs the same button as a busy one. Open in Finder, copy path, or delete (confirm-first). The tree groups the files under three sections — `wip` ("in progress"), `outputs` ("outputs") and `resources` ("resources"). A section is a group heading, not a folder: it shows the count and its one-word description, carries the sync dot of everything inside it, collapses with its chevron and has no actions of its own — it cannot be renamed or moved. Folders under a section are ordinary rows in the same face and size as the file rows (chevron, folder icon, name, count, sync dot, and a hover action strip with "Rename" and "New subfolder" at the right), with a guide line down the left of their children; a top-level folder outside the three sections renders as such a row too. Opening a file swaps the editor into this same right column in place of the node detail — a "← Back" button returns to it. Every file but a Showtime deck offers a Preview | Edit toggle: Markdown and HTML open in Preview (rendered preview) by default, anything else opens in Edit (source editor); switch either way any time. "Save" (Cmd/Ctrl+S) shows up only in Edit, and only once the file is dirty, sitting left of the toggle so its appearing/disappearing never shifts the other buttons. A ⤢ next to it expands the editor to a fullscreen overlay with the same controls (mode toggle, Save, plus ⤡ to collapse back and × to close). A `.showtime` deck (a [Showtime](https://github.com/honzapav/showtime) bundle) opens as the rendered preview the bundle carries once the Showtime integration is on (Settings → Integrations); the preview is read-only and offers "Open in Showtime" when Showtime.app is installed. That button hands Showtime the deck **and** the node: the agent Showtime starts beside the deck connects to the Portuni MCP server with this node as its home (so it appears under the node's Threads tab like a task started from Portuni) and gets the node's mirror as a second working directory. The bearer never travels in the link; Portuni mints a one-time code and Showtime exchanges it over loopback. A refused handoff shows its reason in the preview bar and opens nothing; a Showtime without the `showtime://` deep link asks you to update Showtime. With the integration on and Showtime.app found, "+ New file" is a split button: the chevron offers "New file" and "New presentation". The latter hands Showtime the node's `wip/` and a one-time code (`showtime://new`); Showtime's New Deck screen opens with that folder fixed and the node named, you pick the design system, template and name there, and the bundle it writes shows up under Files through the mirror watcher — the agent beside it is a session on this node, as for "Open in Showtime". Disabled (with the reason) on a node without a mirror on this device: Showtime writes to disk, so there is nowhere to put the deck. Which agent runs beside the deck is Showtime's own setting, not Portuni's agent preset. A bundle saved by a Showtime older than the bundled preview says so instead of rendering.
- **Events** — recent timeline; resolve / supersede inline.
- **Threads** — persistent sessions anchored to this node (`GET /nodes/:id/sessions`), newest-active first: state (running/suspended/closed/archived, archived hidden behind a "Show archived" filter), last activity, CLI + instance (CLI is read from the MCP handshake itself, not a header — populated for Claude Code, Codex and Mistral Vibe alike), the host that ran it (the machine whose sidecar started the latest run — its label on the machine you are asking from, its id when the task ran on a teammate's device; omitted when no run claimed one), the `runner` when the session was started as a task, `waiting_since` when its run is blocked on a question, and write count (size of the session's write scope, which always includes the session's own home node — a session that only ever wrote there still reports 1, not 0). Name defaults to `<node> · <date> <time>` (the time component keeps two same-day sessions on the same node distinguishable) and is enriched from the summary's title when Hand off writes one, but is always renamable inline. A `sessions` row is only ever created once a connection completes its MCP handshake — a client's protocol probe, an aborted connection, or any other non-`initialize` first request never leaves a row behind. A `running` row is never simply dropped: a run that ends for any reason other than Close thread — `PORTUNI_RUN_IDLE_MS` (default 30 min) of inactivity with no turn in flight, a provider limit (the runner's own spend/rate limit, whose message lands in the transcript as a provider error), an error, or a startup sweep finding a row from a process that no longer exists (crash, restart) — *suspends* it instead, and writes no summary: the next message continues the CLI conversation, or, where that conversation cannot be resumed, starts from a summary built at that moment from the transcript on the device that ran the thread. A summary file (`wip/sessions/<id>-handoff.md`) is written only when you ask for one — Hand off and Continue in a new thread — and always by **the device**, which holds the transcript; the central server never writes one. `closed` is reached only by explicitly clicking Close thread (or Close on its row), and `archived` only by the auto-archive sweep of old closed sessions; writing into a closed thread reopens it. The thread's own MCP connection closing is not one of those reasons: an agent that spends half an hour on files without calling a single Portuni tool — or whose connection simply drops — keeps its thread, its run and its transcript, and its next connection binds back to the same thread. A dropped connection does still suspend a **hand-opened CLI**, whose session was that connection and nothing else, with no summary. A suspended row shows whether the underlying CLI conversation is still resumable or will fall back to the summary, and links to the handoff file when Hand off wrote one (or, when this device had no local mirror for the node then, the handoff text is still resumable from — it was simply never written to a file here) — this is informational only now: there is no separate resume action, sending the next message into the thread is what resumes it, `--resume` on the last run when still valid, from the summary otherwise. Each row's status dot doubles as a chip (Running / Waiting on me / Suspended / Done / Archived, "Waiting on me" overriding "Running" while a question is open). A row names the thread and never quotes it: what was said is content, and content is not on the central server to be listed. The tab lists **your** threads on this node and nobody else's — a thread is its owner's, so seeing the node (or holding `manage`) says nothing about the work other people started on it, and there is no owner column to show. "Open chat" jumps to [the task chat](#task-chat) — on a closed row too: the chat shows the thread's history with the composer open, and your next message reopens it (there is no separate resume action any more; Continue in a new thread in the chat header is the deliberate fresh start). Close on a row asks nothing. Every action here is offered on state alone, because every row is yours. Above the rows the tab lists the node's **Handoffs to continue from**: every `wip/sessions/<id>-handoff.md` file tracked on the node, whoever wrote it — including one that arrived here by sync from another machine. Such a file names the thread it came from (the thread's name, its device and its last activity when that thread is one of yours; otherwise just the file name, because a thread is its owner's and a file says nothing about who may see the record), opens in the editor like any other file, and offers **Continue from handoff**: `POST /sessions` with the file's path, which starts a new thread here from what the file says and switches Work to it. The file has to be on this device already — a handoff that has not synced here yet is refused (`HANDOFF_FILE_NOT_HERE`) and nothing is created.

**Arranging files in the Files panel.** Files are dragged between a node's
folders and sections with the mouse, but no drag takes effect right away: it
builds up a **plan**, which is applied only with the "Apply" button. You can
drag a registered file inside the `wip`, `outputs` and `resources` sections,
and a whole folder (every file inside moves with it). A row that cannot be
dragged says why in its tooltip — the file is not registered yet (the
watcher registers it within moments), it lies outside the three sections, or
the node has no mirror on this computer. The target is a folder, a section,
or a file row (which targets the folder that file lies in); a collapsed
folder expands on its own once the cursor rests on it for a moment. A target
that already holds a file of the same name, and a folder dragged into
itself, are refused right on drop, with the reason in the tooltip.

A planned file gets an accent stripe at the left edge of its row, its
original folder struck through and a "move" badge; above the tree a bar
appears reading "3 changes waiting to be applied" with "Discard" and
"Apply" buttons. The bar is there whenever the plan holds anything — both a
planned move and a new (virtual) folder count as changes, so even a plan
holding only a new folder can be dropped with "Discard". "Discard" drops
the whole plan, new folders included, and nothing happens anywhere.
"Apply" goes through the planned moves one by one — a plan without a
single move has nothing to apply and the button is disabled: a file that is
already on the remote is only renamed on Drive (its content is not uploaded
again), a file that has never been pushed changes only its record and its
copy in the mirror and stays in the `push` state at the new path. When a
move fails, the batch stops — what is done stays done, the failed row shows
the reason and the rest of the changes stay in the plan for "Apply again".
The plan belongs to the node on this computer, survives leaving the node
and restarting the app, and is sent nowhere.

**Folders in the Files panel.** Next to "New file" is "New folder": it
opens a form with the path prefilled with a section (`wip/`); Enter or
"Create" creates the folder, Escape or "Cancel" closes the form. An invalid
path (outside the three sections, with `..`, with an empty segment or with a
leading or trailing space), and a path another folder already has — real or
planned — is refused with the reason below the form. On a node with no
mirror on this computer the button is disabled, with the reason in its
tooltip. On hover, a folder row offers the same form under the "New
subfolder" action (the path is prefilled with that folder) and a "Rename"
action: an inline input with the current name, Enter plans the rename,
Escape cancels it, a refusal shows below the row. The `wip`, `outputs` and
`resources` sections have no such actions — they are group headings, not
folders.

Until "Apply", a new folder is only **virtual**: a row in the tree with a
dashed icon, a zero count and a "new" badge; nothing has been created on
disk or on the remote. It becomes real only when the first file ends up in
it once the plan is applied; when the last planned file leaves it again
before that, it disappears from the plan too. Renaming a real folder means
planning a move of every file inside it (the folder itself has no record) —
they all get the "move" badge, the folder shows in the tree under its new
name and the old one disappears; for a virtual folder only its row is
rewritten. A folder holding an unregistered file cannot be renamed, so that
it is never left only half moved; the watcher registers the file within
moments.

The action bar below the pane (non-organization nodes) is "New task" — see [Task chat](#task-chat).
...
"New task" on a node's detail pane (or the "+" on its sub-row in the
left column) opens a thread immediately — one click, no dialog: `POST
/sessions` with just `{ node_id }` creates a `draft` session (a name,
a node, an owner, nothing else yet) and Work's centre column switches to
...
default instance registered for it (Settings → Runners), that instance —
...
sidebar arrangement, Close thread in the chat header, or Close on its row in
the node's Threads tab; and one left
...
**Close thread** and **Hand off** wait for the start too and then act on the run
...
pressed Hand off. The message still lands in the transcript, and Portuni
...
A message is either delivered once or reported as not sent, never both.
When the connection to the sidecar is down as you send (the laptop slept,
the sidecar restarted) and does not come back within 30 seconds, the chat
says the message was not sent and keeps the text in the composer; that
message is then dropped and never delivered later, so sending it again
reaches the agent once. A message that already went out waits for its
answer however long the start takes. When the connection drops while a
message is on its way, the chat reports it at once; that message may have
reached the agent, so check the transcript before sending it again.
...
`state` and `waiting_since` ("Running", "Waiting on me" when a question is
open, "Suspended", "Closed", "Archived"), and the runner/instance.
...
**Hand off** (see below), **Continue in a new thread** (offered any time
...
switches Work to it) and
**Close thread**, which closes the thread without asking: it ends the run
(the same close `interrupt` never does) and moves the thread to Done.
Closing is not the end of it — a closed thread opens from the Threads tab
("Open chat") with its whole history, and writing into it reopens it
exactly the way writing into a suspended thread resumes it (the same
conversation when it still exists, otherwise a summary of the transcript).
Only an unfinished turn is lost, the same as with Stop. All
...
conversation is not. The chat says **"The transcript is on the device X"** with
the name of the machine that has it, the composer is disabled, and the way
to pick the work up here is the round trip below — **Hand off** on X, then
...
carries it — and the thread goes to "Suspended"; the chat then names the
...
writes a handoff file only when you ask for one, with Hand off or
Continue in a new thread, and the chat shows "Summary saved" only
then. On such a suspended thread Hand off writes the file now, from the
transcript this device holds. Hand off refuses, and says why, before
...
**Continue from handoff** is how the other machine picks it up. The node's
Threads tab there lists the node's handoff files as soon as the sync has
...
So the round trip is: **Hand off** here, sync, **Continue from handoff** there —
...
several) and send with Send, where whatever you type answers the
...
mid-tool-call, such as Portuni's scope expansion or write access (Yes
accepts, No declines; Stop closes it; a dialog asking for more than a
...
summary: the handoff file Hand off wrote, or else one built from this
...
Close thread) shows a fresh one. The header names the runner, instance and
...
agent starts has a window of its own and never moves the ring); from 80 % it turns amber and "Continue in a new
...
action of its own; **Continue in a new thread** above is what carries the
...
if the last one ended, Close thread included, `--resume` or from the
summary, the server's own choice; only an archived thread refuses it), `POST
...
/sessions/:id/handoff` (Hand off) and `POST /sessions/:id/continue` — and the
...
minting a second row — this is why a task's Threads row appears the moment
...
`{ id, type: "error", payload: { code, message, params? } }` on failure — the same
...
`suspended`. This is what lets the Threads tab, the Work sidebar and Overview
update live instead of polling. A frame for a node open in Work also refetches that node's thread
...
remote. "<remote> watched, …" means the central server is polling Drive's change feed
...
full check. A line reading "<remote>: watching reports an error – …" (optionally with
"next attempt …", the watcher's `backoff_until`) means that polling is currently
failing — Drive changes are **not** being applied live, and the fallback is
the periodic full sweep every 6 hours. "<remote> watched, …; the periodic check
reports an error – …" is the other way round: live changes still land, but the
periodic full sweep failed on some node (its error is quoted) and is
waiting for its own "next attempt …" — files that only the sweep can pick
up wait with it. "<remote>: no change watching, periodic check only" is not an
...
3. "New task": opens an empty thread right away with the organisation's default runner and instance preselected in the composer; change them there if you want to. Write the first message and the run starts.
