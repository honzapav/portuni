...
**Uzavřít**, which closes the thread without asking: it ends the run
(the same close `interrupt` never does) and moves the thread to Hotové.
Closing is not the end of it — a closed thread opens from the Relace tab
("Otevřít chat") with its whole history, and writing into it reopens it
...
if the last one ended, Uzavřít included, `--resume` or from the
