...
- **Detail pane** — the right column shows the focused node's full detail: owner, responsibilities, data sources, tools, events timeline, files. Same payload `portuni_get_node` returns over MCP, rendered as a panel rather than JSON. The Files tab remembers which folders you collapsed per node, on this device. The header shows the node's ID and its local path (both click-to-copy), an Open-in-Finder button, and — when the folder lives on Google Drive — copy-link and open-on-Drive actions. If the node has no local mirror on this device yet, the header shows a "Create mirror" button that creates it directly.
- **Workspace view** — a node browser with type / status filters, sidebar navigation, and a status footer.
- **Task chat (Work)** — a node's action bar is "New task": one click opens an empty thread (a draft), and your first message is what starts the task — no picker, no required field. The runner and the provider instance are resolved for you (the node's organization default when it has one); model and reasoning effort are per-thread pickers in the composer. See [Working in the app](/guides/working-in-the-app/#task-chat). There is no embedded terminal; an agent runs only as a task.
- **Actors page** — browse and manage actors and assignments.
- **Settings page** — workspaces (create, enable, open a window for one, pick each one's data mode), per-workspace Turso credentials and workspace root, a Runners section (replaces Profiles, #344: detected runner adapters with their install/login status, and provider instances — env vars, typically `CLAUDE_CONFIG_DIR`, with a default assignable per organization — edited through the sidecar's `GET/POST/PATCH/DELETE /runners/instances` REST API instead of the desktop's own `config.json`; see [Runners](/reference/runners/)), an Account section (the interface language — English or Čeština, saved to your account so it applies on every device and takes effect without a reload; the app menu and the browser page shown after Google sign-in follow the language of the focused window — plus Google sign-in and device tokens for a team workspace), a Sync section (informational — Drive sync is configured once on the central server via a Service Account, see [Setting Up Remotes](/guides/setting-up-remotes/); a personal workspace shows a "no remote" note instead), and an MCP Server section with one-click install buttons for Claude Code, Codex, and Mistral Vibe.
...
- **Footer button.** When a newer version is published, the status footer shows `↑ X.Y.Z` on the right. Clicking it opens Settings → General.
- **Settings → General → "Updates".** Shows the current version and, once a check has run, whether it's up to date or a newer version is available. "Check now" checks on demand. "Download and install" downloads and installs the update, with a progress bar; "What's new" links to the GitHub release page. A download or signature-verification failure leaves the running app untouched and shows the error inline — retry by clicking the button again.
