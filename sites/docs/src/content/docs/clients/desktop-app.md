...
- **Detail pane** — the right column shows the focused node's full detail: owner, responsibilities, data sources, tools, events timeline, files. Same payload `portuni_get_node` returns over MCP, rendered as a panel rather than JSON. The Files tab remembers which folders you collapsed per node, on this device. The header shows the node's ID and its local path (both click-to-copy), an Open-in-Finder button, and — when the folder lives on Google Drive — copy-link and open-on-Drive actions. If the node has no local mirror on this device yet, the header shows a "Vytvořit pracovní složku" button that creates it directly.
- **Workspace view** — a node browser with type / status filters, sidebar navigation, and a status footer.
- **Task chat (Práce)** — a node's action bar is "Nový úkol": pick a runner and, when more than one is registered, a provider instance, and the task runs as a chat in Práce's centre column — see [Working in the app](/guides/working-in-the-app/#task-chat-práce). There is no embedded terminal; an agent runs only as a task.
