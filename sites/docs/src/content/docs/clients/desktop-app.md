...
- **Settings page** — workspaces (create, enable, switch, pick each one's data mode), per-workspace Turso credentials and workspace root, a Profily section (spawn profiles — env vars, typically `CLAUDE_CONFIG_DIR`, plus an optional command override — injected into a terminal at launch, with a default assignable per organization), an Account section (Google sign-in and device tokens for central mode), a Synchronizace section (one-click Google Drive connect for local workspaces — see [Working in the App](/guides/working-in-the-app/#synchronizace-google-drive)), and an MCP Server section with one-click install buttons for Claude Code, Codex, and Mistral Vibe.
