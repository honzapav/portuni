...
Central-mode sessions through the sync agent (the desktop app's central-mode sidecar) apply the same rule on the SAME device — `portuni_expand_scope`, `portuni_get_node` and `portuni_get_context` are proxied to the central server, which has no filesystem of its own, so the sync agent's front door overlays `readable`/`readable_path`/`local_path` from its own local mirror registry before returning.
