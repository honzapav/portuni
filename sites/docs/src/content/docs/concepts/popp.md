...
To move a node from one organization to another, call `portuni_move_node` – it rebinds the existing `belongs_to` edge atomically, so the node is never orphaned even transiently. A disconnect-then-connect across organizations is rejected. The invariant is enforced in three layers:
