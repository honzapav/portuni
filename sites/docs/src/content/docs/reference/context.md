...
`local_path` is the node's **real** mirror path whenever this device has one — the home node, a depth-1 neighbour, or any node added mid-session, all the same way. A node with no local mirror on this device has `local_path: null`; read its files with [`portuni_read_file`](/reference/files/). See [disk read scope](/concepts/scope-enforcement/).
