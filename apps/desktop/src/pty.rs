...
// target) and Homebrew are invisible to the sidecar. The embedded terminal
// never noticed because it runs a login shell (`pick_shell`'s `-l`); the
// runner's `claude` detection and the SDK subprocess run from the sidecar's
// own env, so they need the same PATH a login shell would see. Resolved once
// per process (a login shell is ~100 ms), falling back to the inherited
// PATH when the shell cannot answer.
...
mod login_shell_path_tests {
    ...
    fn parses_the_path_between_markers_ignoring_profile_noise() {
        ...
        assert_eq!(parse_marked_path(&stdout).as_deref(), Some("/opt/homebrew/bin:/usr/bin"));
    }

    #[test]
    fn missing_or_empty_markers_yield_none() {
        assert_eq!(parse_marked_path("no markers here"), None);
        assert_eq!(parse_marked_path(&format!("{PATH_MARKER}{PATH_MARKER}")), None);
    }
    ...
    fn merge_keeps_login_order_and_appends_inherited_extras_once() {
        ...
        assert_eq!(merged, "/Users/x/.local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin");
    }
}
