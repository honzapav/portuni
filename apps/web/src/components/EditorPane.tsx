  ...
  return (
    <div className="flex h-full flex-col bg-[var(--color-bg)]">
      <div className="flex min-h-[42px] items-center gap-1.5 border-b border-[var(--color-border)] px-2.5 py-1.5">
        <Button variant="ghost" size="sm" onClick={onClose} title={t(($) => $.editor.back_title)} className="text-muted-foreground">
          <ChevronLeft /> {t(($) => $.editor.back)}
        </Button>
        ...
        <EditorHeaderActions ed={ed} relPath={relPath} mode={mode} onModeChange={onModeChange}>
          <Button variant="ghost" size="icon-sm" onClick={onExpand} title={t(($) => $.editor.expand_title)} className="text-muted-foreground">
            <Maximize2 />
          </Button>
        </EditorHeaderActions>
      </div>
  ...
  return (
    <span className="ml-auto flex shrink-0 items-center gap-1.5">
      {!previewOnly && mode === "edit" && (
        ...
        >
          <Save /> {ed.saving ? t(($) => $.editor.saving) : t(($) => $.editor.save)}
        </Button>
      )}
      ...
      {!ed.conflict && ed.externalChange && (
        <div className="pointer-events-none absolute inset-x-0 top-3 z-20 flex justify-center px-3">
          <div className="pointer-events-auto flex items-center gap-3 rounded-md border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-accent)_14%,var(--color-bg))] px-4 py-2 text-[12.5px] text-[var(--color-text)] shadow-lg">
            <span>{t(($) => $.editor.external_change.message)}</span>
            <Button variant="link" size="sm" onClick={ed.reloadTheirs} className="h-auto p-0">
              {t(($) => $.editor.external_change.reload)}
            </Button>
          </div>
        </div>
      )}
...
function ModeToggle({
  ...
  return (
    <ButtonGroup aria-label={t(($) => $.editor.mode.group_label)}>
      <Button
        variant="outline"
        size="sm"
