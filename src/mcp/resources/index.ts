...
export function registerResources(server: McpServer): void {
  for (const r of RESOURCES) {
    server.registerResource(
      ...
      async (uri) => {
        // Markdown bodies live next to this module. tsc does not copy
        // non-.ts files, so a postbuild step copies them into
        // dist/resources/. import.meta.url then resolves to the
        // co-located file in both dev (tsx, src/resources) and prod
        // (node, dist/resources).
        const fileUrl = new URL(`./${r.file}`, import.meta.url);
