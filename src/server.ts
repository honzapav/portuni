...
async function main() {
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    // /scope?cwd=<abs>&target=<abs> – classifies a write target into the
    // three tiers (current mirror / sibling mirror / outside PORTUNI_ROOT).
    // Used by the optional portuni-guard PreToolUse hook so that every
    // harness gets a uniform decision surface.
    if (url.pathname === "/scope" && req.method === "GET") {
      ...
      try {
        const { classifyWrite, resolvePortuniRoot } = await import("./write-scope.js");
