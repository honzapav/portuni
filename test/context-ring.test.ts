...
describe("contextRingState", () => {
  it("is null with nothing recorded, undefined or a non-number", () => {
    assert.equal(contextRingState(null, null, "en"), null);
    assert.equal(contextRingState(undefined, undefined, "en"), null);
    assert.equal(contextRingState(Number.NaN, 100, "en"), null);
    assert.equal(contextRingState(10, undefined, "en")?.label, "10 tokenů");
  });
  it("percent under the threshold is not a warning; at 80 % it is", () => {
    assert.deepEqual(contextRingState(79_000, 100_000, "en"), {
      used: 79_000,
      max: 100_000,
      fraction: 0.79,
      warn: false,
    ...
    assert.equal(contextRingState(80_000, 100_000, "en")?.warn, true);
    assert.equal(contextRingState(500, 200_000, "en")?.label, "<1 %");
  });
  it("a count past the window still reads 100 %, never more", () => {
    ...
    const s = contextRingState(1_032_223, 1_000_000, "en");
    assert.ok(s);
    assert.equal(s.label, "100 %");
    assert.equal(s.warn, true);
  });
  it("without a max it shows the count", () => {
    const s = contextRingState(12_345, null, "en");
    assert.ok(s);
