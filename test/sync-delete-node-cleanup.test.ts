...
import { registerMirror, getMirrorPath } from "../apps/server/domain/sync/mirror-registry.js";
...
describe("portuni_delete_node purge cleanup", () => {
  it("purge removes the local mirror row on this device", async () => {
    ...
    const { purgeNodeLocalCleanup } = await import("../apps/server/domain/nodes.js");
