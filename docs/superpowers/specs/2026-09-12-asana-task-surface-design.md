...
|---|---|
| An Asana user for the bot: the workspace's existing **Samo Seto** account (its seat type decides whether Založit is available, see the team row); for a fresh workspace a **guest** by default (an e-mail outside the organization's domain; free, can be assigned, comment, hold a PAT, sees only projects it is added to), a member seat or an Enterprise service account when project creation is wanted | created by a workspace admin once; e-mail and PAT stored in Bitwarden "Portuni Asana bot"; PAT into central's `PORTUNI_ASANA_TOKEN`. One account for the whole workspace regardless of how many hosts run tasks; hosts never talk to Asana |
